...
function color_code(code, input_vec){
    ...
    switch_blocks();
    return [parsed_code, statemens_lines];
}
...
function is_local_assignment(obj){
    if(obj.type == 'ExpressionStatement' && obj.expression.type == 'AssignmentExpression' && variable_exist(obj.expression.left.name, variable_dict) > -1){
        return true;
    }
    return false;
}

function insert_to_block(){
...
function call_all_childes(childes){
    ...
    childes.forEach(function(element){
        recursiveBuilding(element);
        if(element.type !== 'VariableDeclaration' && !is_local_assignment(element)){
